...
const SideBar = ({ close, handelRoomVisual, handleVisualizeClick }) => {
  ...
  return (
    <div className=" rounded flex items-center h-full w-fit md:mr-20 lg:mr-28 z-[125] absolute top-0 right-0">
      <div className=" h-full md:h-fit md:mt-[-100px] w-[100vw] md:w-72 3xl:w-[335px] relative p-5 bg-white">
        ...
        <div className="  text-xl font-Inter md:hidden font-semibold mb-3">
          Visualise it
        </div>
        <div
          className={` ${
            visualizerInfo ? " md:hidden" : "md:block"
          } md:bg-black text-sm font-Inter md:font-medium text-[#979797] md:w-[316px] h-[100px] md:absolute top-[-10px] left-[-120%] md:p-[0.7rem]`}
        >
        ...
        <div
          ...
          className=" w-[31px] h-[31px]  bg-black group-hover:block border-[5px] cursor-pointer text-white hidden md:grid place-content-center rounded-full border-white absolute top-[-15px] left-[-15px] text-xl font-bold"
        >
          ?
        </div>
        <div className=" flex justify-between  flex-col  h-full">
            ...
            {roomsData?.length === 0 ? (
              ...
              <>
                {roomsData?.slice(0, 3).map((rm) => (
                  <div
                    key={rm.id}
                    className=" flex cursor-pointer md:rounded w-[100%]  h-28 3xl:h-[165px] 3xl:w-[293px] overflow-hidden items-center gap-2 mb-4"
                  >
                    <img
                      onClick={() => {
          ...
          <div className=" fixed md:relative bottom-5 md:bottom-0 left-0 w-[calc(100%-40px)] md:w-full mx-5 md:mx-0 h-[58px] md:h-28 3xl:h-[165px] cursor-pointer font-Inter md:text-sm font-medium text-[#979797] border md:border-dashed border-black md:border-2 md:border-[#C7C7C7] flex items-center justify-center flex-col gap-3 rounded overflow-hidden 3xl:w-[293px] md:bg-[#F6F6F6]">
            <svg
              className=" hidden md:block h-[28px] 3xl:h-full"
              xmlns="http://www.w3.org/2000/svg"
              width="63"
              height="51"
              viewBox="0 0 63 51"
            ...
            <p className=" hidden md:block text-xs 3xl:text-sm">
              Click to upload an image <br />
              or drag and drop it here.
            </p>
            <p className=" md:hidden">Upload an image</p>
          </div>
        </div>
      </div>
    </div>
