      ...
      <div className='absolute top-[85%] lg:top-[90%] left-[10%] flex w-full gap-4'>
        {
          designerImg.map(({ id, image }) => (
            <img alt={`designer image ${id}`} onClick={() => router.push('/product-details')} className='cursor-pointer xl:h-10 xl:w-10 h-8 w-8 hover:scale-150 effect' key={id} src={image.src} />
          ))
        }
      </div>
    </div>
