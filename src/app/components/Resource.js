...
const Resource = ({resource}) => {
  return (
    <section className='relative overflow-hidden'>
      ...
      <div className='xl:px-[100px] md:px-16 sm:px-10 px-8 xl:my-14 md:my-10 sm:my-7 my-5 relative'>
      <div className='flex items-center justify-end xl:mb-16 md:mb-12 sm:mb-9 mb-7'>
       Showing 55 of 55 releases
      </div>
      <div className='flex gap-4 xl:mb-40  md:flex-row flex-col lg:mb-32 md:mb-24 mb-16'>
       ...
       <div className='xl:w-[75%] md:w-[70%] w-full grid xl:grid-cols-3 md:grid-cols-2 gap-x-5 gap-y-5 md:justify-normal justify-center'>
        {
          resource.map(( {id, image, resourceName, docType, size})=>(
            <ResourcesPageCard key={id} image={image} resourceName={resourceName} docType={docType} size={size} />
          ))
        }
       </div>
      </div>
