...
const ImageClickModal = ({close, setClose}) => {
  ...
  return (
    <div className='max-w[1920px] mx-auto h-auto z-50 w-full fixed top-[0%] bg-[#ffffff7a] flex justify-center items-center'>
      {
        ...
        <div className='2xl:h-[875px] h-screen overflow-scroll md:overflow-hidden flex w-full'>
        ...
        {
          EVENTS_SUB_IMAGES.map(({id, image})=>(      <img onClick={(e)=>{setSelectedImage(image)}} className={`grayscale h-full w-[calc(100%/${allImages})]`} key={id} src={image.src} />
          ))
        }
        </div>
